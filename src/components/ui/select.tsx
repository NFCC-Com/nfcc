    ...
    <SelectPrimitive.Portal>
      ...
      >
        ...
        <SelectPrimitive.Popup
          ...
          className={cn(
            "p-1 bg-popover text-popover-foreground data-open:animate-in data-closed:fade-out-0 data-[open]:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=right]:slide-in-from-left-2 relative z-50 max-h-(--available-height) min-w-(--anchor-width) origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-md border shadow-md",
            className
          )}
          {...props}
        >
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "focus:bg-accent focus:text-accent-foreground relative w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex",
        className
      )}
      {...props}
    >
